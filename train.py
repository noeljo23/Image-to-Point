...
# Use pure PyTorch implementation instead of chamferdist
from utils import ChamferDistancePyTorch as ChamferDistance
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (images, gt_pc, name) in enumerate(dataloader):
            ...
            if cd_loss.item() > 100 or torch.isnan(cd_loss) or torch.isinf(cd_loss):
                print(f"Skipping bad batch {idx}: loss={cd_loss.item()}")
                continue
            loss = cd_loss
        ...
        accelerator.print(
            f"[Test]Epoch {epoch + 1},  loss:{np.mean(loss_history):.4f}")
        ...
        accelerator.log(
            {"test/loss": np.mean(loss_history),
             "cd": cdtable, "test/epoch": epoch + 1}
        )
